...
object InMemoryStore {
  case class LbImpl[F[_], A](lb: ListBuffer[A])(implicit F: Sync[F]) {
    val impl = new InMemoryStore[ListBuffer, F, A] {
      ...
      def retAll(): F[ListBuffer[A]] = F.delay(lb)
    }
  }
  object LbImpl {
...
object Repository {
  ...
  object Impl {
    def empty[L[_], F[_], A](implicit F: Sync[F], S: InMemoryStore[L, F, (String, A)]) =
